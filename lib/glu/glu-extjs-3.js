...
glu.regAdapter('grid', {
    ...
    afterCreate:function (control, viewmodel) {
        ...
        control.getView().onHeaderClick = function (g, index) {
            if (this.headersDisabled || !this.cm.isSortable(index)) {
                return;
            }
            g.stopEditing(true);
            var currentSort = g.store.getSortState() || {direction:'DESC'};
            var order = currentSort.direction == 'ASC' ? 'DESC' : 'ASC';
            this.grid.fireEvent.call(this.grid, "sortrequest", control, {field:this.cm.getDataIndex(index), direction:order});
