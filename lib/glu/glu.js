    ...
    deepApply:function (target, config, noOverwrite) {
        for (var propName in config) {
            ...
            if (glu.isObject(propValue) &&
                propName.indexOf('Tpl', this.length - 3) !== -1 &&
...
glu.Viewmodel = glu.extend(Object, {
     ...
     * @return {*}
     */
    open:function (config, viewMode, animation) {
        config.ns = config.ns || this.ns;
        config.parentVM = config.parentVM || this;
        var win = glu.openWindow(config, viewMode, animation);
...
glu.mreg('liststoreadapter', {
    initMixin : function() {
        ...
        if (this.parentVM[attachTo]) {
            this.on('update', function(store, record, operation, modifiedFieldNames, eOpts) {
                modifiedFieldNames = glu.isArray(modifiedFieldNames)?modifiedFieldNames : [modifiedFieldNames]
